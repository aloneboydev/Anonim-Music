...
async def play(_, message: Message):
    ...
    if not a.can_invite_users:
        await message.reply_text(
        "❌ Saya tidak memiliki izin yang diperlukan untuk melakukan tindakan ini."
        + "\n**Izin:** **INVITE PENGGUNA MELALUI LINK**")
        return
    ...
    try:
        ...
        if b.status == "kicked":
            await message.reply_text(f"❌ {ASSNAME}(@{ASSUSERNAME}) Dibanned di obrolan Group Anda **{chat_title}**\n\nUnban pemblokirannya terlebih dahulu untuk menggunakan Musik")
            return
        ...
        if message.chat.username:
            try: 
                ...
                await message.reply_text(f"❌ **Asisten Gagal Bergabung**\n\n**Reason**:{e}")
                return
        else:
            try:
                ...
                return await message.reply_text(f"❌ **Asisten Gagal Bergabung**\n\n**Alasan**:{e}")       
...
╚ ✨ **Powered by** : [𝐇𝐢𝐫𝐨𝐬𝐡𝐢•](https://t.me/Bisubiarenak)
...
<b>🎧 **Atas Permintaan** :</b> {checking}
<b>⚡ **Powered by** :</b> [𝐇𝐢𝐫𝐨𝐬𝐡𝐢•](https://t.me/Bisubiarenak)
""")
        )   
        os.remove(thumb)
        await CallbackQuery.message.delete()
