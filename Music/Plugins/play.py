...
async def play(_, message: Message):
    ...
    if not a.can_manage_voice_chats:
        ...
        + "\n**Permission:** __KELOLA CHAT SUARA__")
        return
    ...
    if not a.can_invite_users:
        await message.reply_text(
        "❌ Saya tidak memiliki izin yang diperlukan untuk melakukan tindakan ini."
        + "\n**Permission:** __UNDANG PENGGUNA MELALUI LINK__")
        return
    if not a.can_restrict_members:
        await message.reply_text(
        "❌ Saya tidak memiliki izin yang diperlukan untuk melakukan tindakan ini."
        + "\n**Permission:** __BAN PENGGUNA__")
        return
    try:
        b = await app.get_chat_member(message.chat.id , ASSID) 
        if b.status == "kicked":
            await message.reply_text(f"❌ {ASSNAME}(@{ASSUSERNAME}) Dibanned Di Group Anda **{chat_title}**\n\nUnban pemblokirannya terlebih dahulu untuk menggunakan Musik")
            return
    ...
    if audio:
        ...
        if views == "None":
            return await mystic.edit_text("❌ Maaf! Video langsung tidak Didukung")
        semxbabes = (f"Downloading {title[:60]}")
...
""",    
    ...
    if await is_active_chat(chat_id):
        ...
        photo=thumb,
        reply_markup=InlineKeyboardMarkup(buttons),    
        caption=(f"🏷 <b>__Nama:__</b>[{title[:80]}]({link}) \n⏱️ <b>__Duration:__</b> `{duration}` `Menit` \n👤 <b>Atas permintaan:__</b>{checking}") \n✨ <b>Dikelola oleh:</b> [𝙂𝙇𝙄𝙏𝙏𝙀𝙍•](https://t.me/Biarenakliatnyaaaa)
    )   
        return await mystic.delete()
...
async def startyuplay(_,CallbackQuery): 
    ...
    if await is_active_chat(chat_id):
        ...
        photo=thumb,
        caption=(f"🏷 <b>Nama: </b>[{title[:80]}]({url}) \n⏱️ <b>Duration:</b> `{duration}` `Menit`\n💡 <b>Info:</b> [More Information](https://t.me/{BOT_USERNAME}?start=info_{id})\n👤 <b>Atas permintaan: </b>{checking} \n#️⃣ <b>Posisi antria:__</b> <b>#{position}!</b>"),
        reply_markup=InlineKeyboardMarkup(buttons)
    )
        os.remove(thumb)
        await CallbackQuery.message.delete()       
